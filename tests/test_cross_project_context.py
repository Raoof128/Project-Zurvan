...
@patch("scripts.graph_context.expand_graph_context")
@patch("scripts.cross_project_context.get_federated_projects")
@patch("scripts.cross_project_context.cross_project_search")
def test_build_federated_context(mock_search, mock_get_proj, mock_expand):
    ...
    mock_expand.return_value = [
        {"depth": 1, "title": "Node B", "node_type": "concept", "relation": "links to"}
    ]
    ...
    # Graph expansion runs against the TARGET project's graph DB.
    assert mock_expand.call_args.kwargs["db_path"].startswith("/tmp/p1")
